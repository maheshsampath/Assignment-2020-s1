...
@Provider
public class AdminLoginFilter implements ContainerRequestFilter{

	UserRepository repo=new UserRepository();
	private static final String AUTHORIZATION_HEADER_KEY="Authorization";
	private static final String AUTHORIZATION_HEADER_PREFIX="Basic";
	private static final String SECURED_URL_PREFIX="adminlogin";
	@Override
	public void filter(ContainerRequestContext requestContext) throws IOException {
		if(requestContext.getUriInfo().getPath().contains(SECURED_URL_PREFIX)) {
			...
			if(AuthHeader!=null && AuthHeader.size()>0) {
				...
				UserModel user;
				try {
					user = repo.getAdminLogin(userName, password);
					if(user.getId()!=0) {
						return;
					}
					else {
				...
				Response unAuthorizeResponse=Response
						.status(200)
						.entity("user cannot Access to this page")
						.build();
				requestContext.abortWith(unAuthorizeResponse);
			}
		}
